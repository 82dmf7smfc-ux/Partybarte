...
`sessions/` holds one starting prompt per driver session, in build order. Each
session writes the next one's file before finishing, so the work can be picked up
on a different day or a different machine.
