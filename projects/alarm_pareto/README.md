...
This is one project inside the Partybarte repository. It lives in
`projects/alarm_pareto`. The repository read me at the root explains the shared
setup, and the rules every project here follows.
...
a pre-built package file. Then run this from the repository root:
...
away from anything you install later. There is one environment for the whole
repository, shared by every project in it, so you only do this once.

If you got here from the zip package instead of the repository, everything you
need is already in the folder you unzipped. Run the same command from there.

## Running the tool

Run this from the folder that holds the `alarm_pareto` package folder.

From the repository, that is `projects\alarm_pareto`, and the environment is two
levels up at the root:

```
cd projects\alarm_pareto
..\..\.venv\Scripts\python.exe -m alarm_pareto.main --input path\to\your_log.csv --vendor amat
```

From the unzipped download package, everything sits together, so it is shorter:

```
.venv\Scripts\python.exe -m alarm_pareto.main --input path\to\your_log.csv --vendor amat
```

The options below are the same either way.
...
From the repository root, this runs every project's tests, including this one:

```
.venv\Scripts\python.exe -m pytest -q
```

To run only this project's tests, name its folder:

```
.venv\Scripts\python.exe -m pytest -q projects\alarm_pareto
```

From the unzipped download package, plain `pytest -q` runs them.
...
These are shared by the whole repository, so they sit at the root, not in this
folder.
